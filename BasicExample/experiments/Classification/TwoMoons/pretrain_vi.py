...
# net.train_model(train_loader, epochs=500, n_datapoints=n_datapoints, samples=5, method=LikApprox.MONTECARLO)
...
laplace_feature_extractor = net.feature_extractor.add_last_layer(net.weight_distribution.get_mu())



n_test_datapoints = 100
X_test, X1_test, X2_test = create_test_points(-20, 21, n_test_datapoints)
fig, ax = plt.subplots(figsize=(10,6))

with torch.no_grad():
    # lik = net(X_test, method = PredictApprox.MONTECARLO, samples=1000)
    lik = net.forward_ML(X_test)
    # lik = laplace_feature_extractor(X_test)

pred_test = torch.argmax(net.forward_ML(x), dim=1)
print("accuracy", torch.mean((pred_test == y).float()).item())
pred_test = torch.argmax(laplace_feature_extractor(x), dim=1)
print("accuracy", torch.mean((pred_test == y).float()).item())
